...
const MODULE_PATH: &[&str] = &["signal_agent", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let source =
            fs::read_to_string(&source_path).expect("read ordinary Agent Interface source");
        ...
        .expect("assemble authority-approved ordinary Agent Interface transaction");
        let rust = rust_logos();
        let type_paths = ProducerRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project ordinary Agent Interface from the verified transaction")
            .write_or_check("SIGNAL_AGENT_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in ordinary Agent Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("signal-agent")
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("ordinary Agent bootstrap catalog is complete")
}
